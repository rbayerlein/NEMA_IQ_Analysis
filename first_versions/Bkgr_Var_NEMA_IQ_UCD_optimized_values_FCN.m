...
function Bkgr_Var_NEMA_IQ_UCD_optimized_values_FCN(img_name_folder, activity_ratio, voxel_size, img_size, spheres_diam, sphere_offset_vector, ROI_misalign)
...
img_name_raw = [img_name_folder, 'NEMA_IQ_Ground_Truth_', num2str(voxel_size), 'iso.'];

num_iter=1; % total number of iterations

voxel_size_scaling_factor=2.85/voxel_size;
...
%slice to show
% center_slice = 340;
% center_slice = round(center_slice*voxel_size_scaling_factor);
...
xy_range=10;         % given as number of steps of size xy_stepsize
xy_stepsize=0.5;   % measured in pixels

artif_shift = 1.0;  % to compensate for weird matlab alignment issue in y-direction
misalignment = ROI_misalign; % misalgnment on purpose (for testing)

% activity contrast
act_contrast = activity_ratio;
...
spheres_center(4,:)= [ sin(pi/6)*57.2, cos(pi/6)*57.2 ];
spheres_center(5,:)= [-sin(pi/6)*57.2, cos(pi/6)*57.2 ];
spheres_center(6,:)= [-57.2, 0 ];
spheres_center(1,:)= [-sin(pi/6)*57.2, -cos(pi/6)*57.2 ];
spheres_center(2,:)= [ sin(pi/6)*57.2, -cos(pi/6)*57.2 ];
spheres_center(3,:)= [ 57.2, 0];
...
% phantom_center = [120.5, 121];
phantom_center = [img_size(1)/2, img_size(2)/2 + artif_shift + misalignment];
% phantom_center = phantom_center*2;

sphere_offset_vector_2D = zeros(1,2);
sphere_offset_vector_2D(1) = -sphere_offset_vector(1);
sphere_offset_vector_2D(2) = sphere_offset_vector(2);

spheres_center = spheres_center/voxel_size  + phantom_center + sphere_offset_vector_2D
    ...
    for f_num = 1 : num_iter    %% running over iterations
        ...
        for x_shift = -xy_range : 1 : xy_range     
            ...
            %fprintf('current shift:\tx=%d\ty=%d\n', x_shift, y_shift);
                ...
                fprintf('%d, x=%d y=%d\n',contrast_tmp(f_num, 1), spheres_center(1,1) + x_shift*xy_stepsize, spheres_center(1,2) + y_shift*xy_stepsize);
...
best_xy_values
for s = 1 : 6
    spheres_center(s,2) =spheres_center(s,2) + xy_stepsize * best_xy_values(2,num_iter,s);
    spheres_center(s,1) =spheres_center(s,1) + xy_stepsize * best_xy_values(1,num_iter,s);
end

drawcircle('Center',spheres_center(1,:),'Radius',spheres_diam(1)/2, 'Color', 'r');
drawcircle('Center',spheres_center(2,:),'Radius',spheres_diam(2)/2, 'Color', 'r');
drawcircle('Center',spheres_center(3,:),'Radius',spheres_diam(3)/2, 'Color', 'r');
drawcircle('Center',spheres_center(4,:),'Radius',spheres_diam(4)/2, 'Color', 'r');
drawcircle('Center',spheres_center(5,:),'Radius',spheres_diam(5)/2, 'Color', 'r');
drawcircle('Center',spheres_center(6,:),'Radius',spheres_diam(6)/2, 'Color', 'r');

fclose ('all');
disp('done CRC analysis');
disp('======================================================');
