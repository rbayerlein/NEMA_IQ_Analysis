...
function NEMA_IQ_Ground_Truth_Creator_FCN(output_folder, activity_ratio, vox_size, subsample_factor, img_size, spheres_diam, sphere_offset_vector)

%% parameters

desired_voxel_size = vox_size;
...
% subsample_factor = 20;   % factor for subsampling a voxel; applied per dimension;
...
spheres_center = round(spheres_center/voxel_size) + phantom_center - sphere_offset_vector; % - [0.5 0.5 0]
                    ...
                    if y > phantom_center(2)-BG_size_y/2 && y < phantom_center(2) + BG_size_y/2
                        ...
                        for s = 1 : 6   
                            if sqrt(power(x-spheres_center(s,1),2) + power(y-spheres_center(s,2),2) + power(ax-spheres_center(s,3),2)) <= spheres_diam(s)/2 + 0.5*sqrt(3)
                                ...
                                if grnd_trth_img(x,y,ax) > 1.0
                                    total_hot_activity = total_hot_activity + grnd_trth_img(x,y,ax)-1 ;
                                end
...
% total_hot_activity=total_hot_activity*power(voxel_size,3)
...
fname_out = [output_folder, '/NEMA_IQ_Ground_Truth_', num2str(voxel_size), 'iso.1'];
fid_out = fopen(fname_out, 'wb');
fwrite(fid_out, grnd_trth_img, 'float');
fprintf('image saved as: %s\n', fname_out);
fclose(fid_out);
disp('done image generation');
